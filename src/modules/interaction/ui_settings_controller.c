...
static size_t array_len_u8(const uint8_t *values, size_t bytes)
{
	(void)values;
	return bytes / sizeof(uint8_t);
}

static bool cycle_u8(uint8_t *value, const uint8_t *values, size_t count, uint8_t fallback)
{
...
static bool cycle_u16(uint16_t *value, const uint16_t *values, size_t count, uint16_t fallback)
{
	for (size_t i = 0; i < count; i++) {
		if (*value == values[i]) {
			*value = values[(i + 1U) % count];
...
static bool cycle_i16(int16_t *value, const int16_t *values, size_t count, int16_t fallback)
{
	for (size_t i = 0; i < count; i++) {
		if (*value == values[i]) {
			*value = values[(i + 1U) % count];
...
static void process_env_settings(const ui_settings_controller_state_t *state)
{
	if (*state->focus == 0U) {
		*state->env_voice_on = !*state->env_voice_on;
		...
		(void)cycle_u8(state->env_sample_s, values, array_len_u8(values, sizeof(values)), 10);
		state->save_settings();
	} else if (*state->focus == 3U) {
		static const int16_t values[] = { 0, 5, 10, 15, 20 };
		(void)cycle_i16(state->env_temp_low_c, values, sizeof(values) / sizeof(values[0]), 10);
		...
		(void)cycle_i16(state->env_temp_high_c, values, sizeof(values) / sizeof(values[0]), 35);
		...
		(void)cycle_u16(state->env_humi_low_percent, values, sizeof(values) / sizeof(values[0]), 30);
		...
		(void)cycle_u16(state->env_humi_high_percent, values, sizeof(values) / sizeof(values[0]), 80);
		...
		(void)cycle_u16(state->env_lux_low, values, sizeof(values) / sizeof(values[0]), 20);
		...
		(void)cycle_u16(state->env_lux_high, values, sizeof(values) / sizeof(values[0]), 1000);
...
static void process_low_settings(const ui_settings_controller_state_t *state)
{
	if (*state->focus == 0U) {
		*state->low_use_24h = !*state->low_use_24h;
		...
		(void)cycle_u16(state->low_enter_absent_s, values, sizeof(values) / sizeof(values[0]), 60);
	} else {
		static const uint16_t values[] = { 1, 3, 5, 10, 30 };
		(void)cycle_u16(state->low_exit_present_s, values, sizeof(values) / sizeof(values[0]), 3);
	}
	state->save_settings();
}
