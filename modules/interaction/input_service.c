...
static void input_task(void *arg)
{
	...
	for (;;) {
		...
		if ((now_us - s_last_edge_us[key_index]) < 20000) {
			continue;
		}

		s_last_edge_us[key_index] = now_us;
		s_key_levels[key_index] = gpio_get_level((gpio_num_t)gpio_num) != 0;

		app_key_event_t event = {
			.key_index = (size_t)key_index,
			.pressed = !s_key_levels[key_index],
			.timestamp_us = now_us,
		};
